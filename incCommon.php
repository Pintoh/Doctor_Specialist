	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			...
			'patient_visits' => array('Patient visits', '', 'resources/table_icons/cheque.png', 'None'),
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'patient_visits' => "`patient_visits`.`id` as 'id', IF(    CHAR_LENGTH(`patients1`.`last_name`) || CHAR_LENGTH(`patients1`.`first_name`), CONCAT_WS('',   `patients1`.`last_name`, ', ', `patients1`.`first_name`), '') as 'patient', `patient_visits`.`type_of_visit` as 'type_of_visit', `patient_visits`.`prescription` as 'prescription', `patient_visits`.`diagnosis` as 'diagnosis', `patient_visits`.`comments` as 'comments', if(`patient_visits`.`observation_date`,date_format(`patient_visits`.`observation_date`,'%m/%d/%Y'),'') as 'observation_date', TIME_FORMAT(`patient_visits`.`observation_time`, '%r') as 'observation_time'",
			...
			'medical_records' => "`medical_records`.`id` as 'id', IF(    CHAR_LENGTH(`patients1`.`last_name`) || CHAR_LENGTH(`patients1`.`first_name`), CONCAT_WS('',   `patients1`.`last_name`, ', ', `patients1`.`first_name`), '') as 'patient', `medical_records`.`document` as 'document', `medical_records`.`description` as 'description'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'patients' => "`patients` ",
			'patient_visits' => "`patient_visits` LEFT JOIN `patients` as patients1 ON `patients1`.`id`=`patient_visits`.`patient` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('online_clinic_management_system_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('online_clinic_management_system_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['online_clinic_management_system_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['online_clinic_management_system_rememberMe']);
	...
	*/
	...
	function get_embed_googlemap($url, $max_width = '', $max_height = '', $retrieve = 'html'){
		...
		if(preg_match($coords_regex, $url_parts['path'] . '?' . $url_parts['query'], $m)){
			...
			$api_key = 'AIzaSyBywCQY3P3ZDQmQror1i9mOGPTl1hGTAS4';
	...
	function NavMenus($options = array()){
		...
		if(is_array($arrTables)){
			foreach($arrTables as $tn => $tc){
				...
				$tChkHL = array_search($tn, array('patient_visits','medical_records'));
