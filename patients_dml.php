...
function patients_insert(){
	...
	if($_REQUEST['SelectedID']){
		$res = sql("select * from patients where id='" . makeSafe($_REQUEST['SelectedID']) . "'");
	...
	if(function_exists('patients_after_insert')){
		...
		$args=array();
		if(!patients_after_insert($data, getMemberInfo(), $args)){ return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID); }
	}

	// mm: save ownership data
	sql("insert ignore into membership_userrecords set tableName='patients', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);
...
function patients_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	// child table: patient_visits
	$res = sql("select `id` from `patients` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `patient_visits` where `patient`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "patient_visits", $RetMsg);
		return $RetMsg;
...
function patients_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	if($dvprint){
		$templateCode = @file_get_contents('./templates/patients_templateDVP.html');
	}else{
		$templateCode = @file_get_contents('./templates/patients_templateDV.html');
	}
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode=str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return patients_validateData();"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(last_name)%%>', htmlspecialchars($row['last_name'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(last_name)%%>', urlencode($urow['last_name']), $templateCode);
		$templateCode=str_replace('<%%VALUE(first_name)%%>', htmlspecialchars($row['first_name'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(first_name)%%>', urlencode($urow['first_name']), $templateCode);
		$templateCode=str_replace('<%%VALUE(gender)%%>', htmlspecialchars($row['gender'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(gender)%%>', urlencode($urow['gender']), $templateCode);
		$templateCode=str_replace('<%%VALUE(sexual_orientation)%%>', htmlspecialchars($row['sexual_orientation'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(sexual_orientation)%%>', urlencode($urow['sexual_orientation']), $templateCode);
		$templateCode=str_replace('<%%VALUE(birth_date)%%>', @date('m/d/Y', @strtotime(htmlspecialchars($row['birth_date'], ENT_QUOTES, 'iso-8859-1'))), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(birth_date)%%>', urlencode(@date('m/d/Y', @strtotime(htmlspecialchars($urow['birth_date'], ENT_QUOTES, 'iso-8859-1')))), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(image)%%>', htmlspecialchars($row['image'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		...
		$templateCode=str_replace('<%%VALUE(state)%%>', htmlspecialchars($row['state'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(state)%%>', urlencode($urow['state']), $templateCode);
		$templateCode=str_replace('<%%VALUE(zip)%%>', htmlspecialchars($row['zip'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(zip)%%>', urlencode($urow['zip']), $templateCode);
		$templateCode=str_replace('<%%VALUE(home_phone)%%>', htmlspecialchars($row['home_phone'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(home_phone)%%>', urlencode($urow['home_phone']), $templateCode);
		$templateCode=str_replace('<%%VALUE(work_phone)%%>', htmlspecialchars($row['work_phone'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(work_phone)%%>', urlencode($urow['work_phone']), $templateCode);
		$templateCode=str_replace('<%%VALUE(mobile)%%>', htmlspecialchars($row['mobile'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(mobile)%%>', urlencode($urow['mobile']), $templateCode);
		$templateCode=str_replace('<%%VALUE(tobacco_usage)%%>', htmlspecialchars($row['tobacco_usage'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(tobacco_usage)%%>', urlencode($urow['tobacco_usage']), $templateCode);
		$templateCode=str_replace('<%%VALUE(alcohol_intake)%%>', htmlspecialchars($row['alcohol_intake'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(alcohol_intake)%%>', urlencode($urow['alcohol_intake']), $templateCode);
		$templateCode=str_replace('<%%VALUE(history)%%>', htmlspecialchars($row['history'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(history)%%>', urlencode($urow['history']), $templateCode);
		if($dvprint){
			$templateCode = str_replace('<%%VALUE(surgical_history)%%>', nl2br(htmlspecialchars($row['surgical_history'], ENT_QUOTES, 'iso-8859-1')), $templateCode);
		}else{
			$templateCode = str_replace('<%%VALUE(surgical_history)%%>', htmlspecialchars($row['surgical_history'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		}
		$templateCode=str_replace('<%%URLVALUE(surgical_history)%%>', urlencode($urow['surgical_history']), $templateCode);
		if($dvprint){
			$templateCode = str_replace('<%%VALUE(obstetric_history)%%>', nl2br(htmlspecialchars($row['obstetric_history'], ENT_QUOTES, 'iso-8859-1')), $templateCode);
		}else{
			$templateCode = str_replace('<%%VALUE(obstetric_history)%%>', htmlspecialchars($row['obstetric_history'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		}
		$templateCode=str_replace('<%%URLVALUE(obstetric_history)%%>', urlencode($urow['obstetric_history']), $templateCode);
		if($dvprint){
			$templateCode = str_replace('<%%VALUE(genetic_diseases)%%>', nl2br(htmlspecialchars($row['genetic_diseases'], ENT_QUOTES, 'iso-8859-1')), $templateCode);
		}else{
			$templateCode = str_replace('<%%VALUE(genetic_diseases)%%>', htmlspecialchars($row['genetic_diseases'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		}
		$templateCode=str_replace('<%%URLVALUE(genetic_diseases)%%>', urlencode($urow['genetic_diseases']), $templateCode);
		$templateCode=str_replace('<%%VALUE(contact_person)%%>', htmlspecialchars($row['contact_person'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(contact_person)%%>', urlencode($urow['contact_person']), $templateCode);
		if($dvprint){
			$templateCode = str_replace('<%%VALUE(other_details)%%>', nl2br(htmlspecialchars($row['other_details'], ENT_QUOTES, 'iso-8859-1')), $templateCode);
		}else{
			$templateCode = str_replace('<%%VALUE(other_details)%%>', htmlspecialchars($row['other_details'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		}
		$templateCode=str_replace('<%%URLVALUE(other_details)%%>', urlencode($urow['other_details']), $templateCode);
		if($AllowUpdate || $AllowInsert){
			$templateCode=str_replace('<%%HTMLAREA(comments)%%>', '<textarea name="comments" id="comments" rows="5">'.htmlspecialchars($row['comments'], ENT_QUOTES, 'iso-8859-1').'</textarea>', $templateCode);
		}else{
		...
		$templateCode=str_replace('<%%VALUE(filed)%%>', htmlspecialchars($row['filed'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(filed)%%>', urlencode($urow['filed']), $templateCode);
		$templateCode=str_replace('<%%VALUE(last_modified)%%>', htmlspecialchars($row['last_modified'], ENT_QUOTES, 'iso-8859-1'), $templateCode);
