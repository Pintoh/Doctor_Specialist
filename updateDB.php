<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('patient_visits', "create table if not exists `patient_visits` (   `id` INT unsigned not null auto_increment , primary key (`id`), `patient` INT unsigned , `type_of_visit` VARCHAR(40) not null default 'Medical examination' , `prescription` TEXT , `diagnosis` TEXT , `comments` TEXT , `observation_date` DATE , `observation_time` TIME ) CHARSET latin1", $silent);
		setupIndexes('patient_visits', array('patient'));
		...
		setupTable('medical_records', "create table if not exists `medical_records` (   `id` INT unsigned not null auto_increment , primary key (`id`), `patient` INT unsigned , `document` VARCHAR(40) , `description` TEXT ) CHARSET latin1", $silent);
